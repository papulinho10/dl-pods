...
Aguardo confirmação!`;
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed top-0 right-0 h-full w-full md:max-w-md bg-street-dark border-l border-white/10 z-[80] transform transition-transform duration-300 ease-out shadow-2xl flex flex-col ${
          isCartOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-white/10 bg-street-black flex-shrink-0">
          ...
          <div className="p-4 md:p-6 space-y-4 md:space-y-6">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-6 py-20">
                ...
                <button onClick={toggleCart} className="text-yellow-400 border-b border-yellow-400 pb-1 text-sm font-bold uppercase tracking-widest">
                  Voltar à Loja
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-4">
                  {cart.map((item) => (
                    <div key={item.id} className="flex gap-4 bg-white/5 p-3 rounded-sm border border-transparent hover:border-white/10 transition-colors">
                      <div className="w-20 h-24 bg-street-gray flex-shrink-0 relative overflow-hidden">
                      ...
                      <div className="flex-1 flex flex-col justify-between py-1">
                        ...
                        <div className="flex justify-between items-end">
                          <div className="flex items-center gap-3 bg-black/40 px-2 py-1 rounded-full">
        ...
        {/* Footer / Checkout */}
        <div className="p-6 border-t border-white/10 bg-street-black safe-area-bottom flex-shrink-0">
          ...
          <button 
            ...
            className={`w-full py-4 md:py-5 font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all duration-300 ${
              isCheckoutEnabled 
